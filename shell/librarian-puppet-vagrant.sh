...
if [ "${FOUND_GIT}" -ne '0' ] && [ ! -f /.viewone-stuff/librarian-puppet-installed ]; then
...
if [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
    if [[ ! -f /.viewone-stuff/librarian-base-packages ]]; then
        ...
        touch /.viewone-stuff/librarian-base-packages
    fi
fi

if [ "${OS}" == 'ubuntu' ]; then
    if [[ ! -f /.viewone-stuff/librarian-libgemplugin-ruby ]]; then
        ...
        touch /.viewone-stuff/librarian-libgemplugin-ruby
    fi

    if [ "${CODENAME}" == 'lucid' ] && [ ! -f /.viewone-stuff/librarian-rubygems-update ]; then
        ...
        touch /.viewone-stuff/librarian-rubygems-update
    fi
fi

if [[ ! -f /.viewone-stuff/librarian-puppet-installed ]]; then
    ...
    touch /.viewone-stuff/librarian-puppet-installed
else
...
if [[ "${OS}" == 'centos' ]]; then
    ...
    git clone https://github.com/viewone/puppetlabs-git.git /etc/puppet/modules/git
