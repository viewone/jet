...
if [[ ! -f /.viewone-stuff/update-puppet ]]; then
    if [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
        ...
        wget --quiet --tries=5 --timeout=10 -O "/.viewone-stuff/puppetlabs-release-${CODENAME}.deb" "http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        echo "Finished downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"

        dpkg -i "/.viewone-stuff/puppetlabs-release-${CODENAME}.deb" >/dev/null
        ...
        touch /.viewone-stuff/update-puppet
        echo "Created empty file /.viewone-stuff/update-puppet"
    elif [ "${OS}" == 'centos' ]; then
